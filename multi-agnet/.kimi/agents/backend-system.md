...
**你的代码工作目录**：`代码工作目录`（由项目经理在本次任务的 prompt 中指定，如 `./backend/order-service/`）
...
- ✅ **可读写**：`代码工作目录` 下的所有代码文件（业务代码、测试、配置等）
- ✅ **只读**：`文档中心目录/design/PRD.md` — 产品需求
- ✅ **只读**：`文档中心目录/design/api-contracts/` 下属于你的微服务的接口契约文档
...
2. **所有文件读取**（除文档中心外）也必须限制在 `代码工作目录` 内
3. **禁止擅自修改接口契约**：契约模糊或不合理处，记录下来等待项目经理协调，不得单方面修改 `文档中心目录/design/api-contracts/` 下的文件
...
开发前请先阅读 `代码工作目录/pom.xml` 和 `代码工作目录/README.md` 了解模块划分。
