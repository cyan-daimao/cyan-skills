---
name: ddbd
description: DDBD 简化版DDD架构规范，用于后端代码生成时强制遵守四层架构、充血模型、转换链路
---
...
| **Application** | 流程编排、业务组装、业务校验，不涉及数据库操作 | BO |
...
**类型转换**：DO ↔ Domain Java代码推荐用 MapStruct 等映射工具自动生成,每层的类型用所在层级区分如: UserAdapterConvert,UserAppConvert,UserInfraConvert;
...
### Adapter
```
根据restful风格,{id}是主键可以作为地址占位符，如果你写RPC接口或对外提供SASS服务的则更推荐使用可读性高的唯一标识作为地址占位符。
Controller {
...
| 文件                        | 公司/项目             | 技术栈                                 |
|----------------------------|-------------------|-------------------------------------|
| examples/java/cyan         | cyan公司 - 内部管理系统  | Java21 + SpringBoot3 + MyBatis-Plus |

## 示例选择流程（强制执行）

当用户要求生成后端代码时，**必须先执行以下步骤**，不可跳过：

1. **询问用户**是否需要参考已有示例代码，提供以下选项：
   - 逐个列出上表中所有可用示例（包含公司/项目名和技术栈信息）
   - "不使用示例"选项
2. 用户选择后：
   - 选择示例：仅读取用户所选示例的源码文件（禁止读取未选择的示例），然后结合示例风格生成代码
   - 不使用示例：仅按本规范中的代码模板生成代码
3. 如果用户没有明确选择，**禁止读取任何示例文件**
